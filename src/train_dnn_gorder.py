...
def train_input_fn(X_train, Y_train, X_norm, set_size, shuffle=False, batch_size=2):
    """
    ...
    def data_generator(X_train, Y_train, X_norm):
        """
        A generator to generate the training data
        """
        n_sample = X_train.shape[0]
        p = np.zeros(shape=(n_sample, 1)) + 1.0 / n_sample
        p = p / np.sum(p)
        p = p[:,0]
        while True:
            #Importance sampling
    ...
    ds = tf.data.Dataset.from_generator(lambda: data_generator(X_train, Y_train, X_norm), 
...
def main(args):
  ...
  # Evaluation
  eval_input_fn, initial_p = build_eval_input_fn(n_eval_data = 1000, n_classes = n_classes, set_size = w-1, 
    X_train = train_data, Y_train = train_labels, X_norm = train_norm)
